...
exports.createTab = functions
    ...
    if (!Array.isArray(members) || members.length < 1) {
        throw new functions.https.HttpsError('invalid-argument', 'At least 1 member required');
    }
    if (!Array.isArray(rotationOrder) || rotationOrder.length !== members.length) {
        throw new functions.https.HttpsError('invalid-argument', 'rotationOrder must match members');
    }
    // Tier check — fetch user's isPro server-side
    ...
    if (!isPro && members.length > _constants_1.FREE_TIER_MEMBER_LIMIT) {
        throw new functions.https.HttpsError('permission-denied', `Free tier limited to ${_constants_1.FREE_TIER_MEMBER_LIMIT} members`);
    }
    const tabRef = db.collection('tabs').doc();
    await db.runTransaction(async (t) => {
        await (0, _rateLimit_1.checkRateLimit)(t, db, uid, 'createTab');
        t.set(tabRef, {
            name,
            createdBy: uid,
            members,
